...
/* pipeline 관련 함수들*/
void treatPipeLine(char** argv, int readableEnd, int idx); // pipeLine 처리해주는 함수
...
void eval(char *cmdline) 
{
   ...
   if(isPipe){
      ...
      if(!replaceMark(argv)) addHistory(cmdline);
      
      treatPipeLine(argv, STDIN_FILENO, 0);
...
/* !!, !# replace한 명령어들은 argv에 띄어쓰기가 있을 수 있음. 
ex) !! == cat historyLog.txt 
이 함수에서 이에 대한 처리를 해줌
*/
int parseSubArgv(char ** subArgv, char * tmpCmd){
   char *delim;         /* Points to first space delimiter */
...
void treatPipeLine(char** argv, int readableEnd, int idx){
   int istherePipe = 0;
   int nxt_idx;
   ...
   if(!istherePipe){
      ...
      if((pid=Fork()) == 0){
         ...
         Close(readableEnd);
         Close(fds[1]);
         waitpid(-1, &status, WUNTRACED); // child reap
         treatPipeLine(argv, fds[0], nxt_idx);
